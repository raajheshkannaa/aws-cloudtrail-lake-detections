...
from base_helpers import deep_get
...
def main(event, context):
	...
	event_name = 'UpdateProjectVisibility'
			...
			for k,v in result.items():
				if k == 'eventJson':			
					...
					if deep_get(event, "requestParameters", "projectVisibility") == "PUBLIC_READ":
						...
						message = "AWS CodeBuild Project *{}* made public by *{}* in account *{}* in region *{}*".format(
							deep_get(event, "requestParameters", "projectArn"),
							deep_get(event, "userIdentity", "arn"),
							deep_get(event, "recipientAccountId"),
							deep_get(event, "awsRegion")
						)
